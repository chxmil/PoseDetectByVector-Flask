...
def check_bowed_pose(left_eye_y, right_eye_y, left_ear_y, right_ear_y , max_distance_bow):
    ...
    return ((distance_bow / max_distance_bow) * 100) , max_distance_bow
...
def calculate_posture(detect_status, head_turn, angle_degrees):
    if head_turn < (((12.673 * (angle_degrees) ** 2) - (4435.9 * angle_degrees)) + (388293)) * 0.5:
        if detect_status == 1:
            return "warning!"
        if detect_status == 0:
